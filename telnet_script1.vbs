...
'  1、在特权模式下，通过show run | in hostn来提取hostname信息；
...
Sub Main
    ...
    DO While file1.AtEndOfStream <> True
	   ...
	   crt.Screen.Send "show run | in hostn" & vbCr 
	   Result = crt.Screen.ReadString("#")	   
       
	   '第一种用换行来分割结果，获取hostname R1,但是返回的是数组的元素，无法加入到文件名中，需要Mid函数再次提取
	   'strHN = split(Result,vbCr)(1) 
       msgbox Mid(Result,20)
	   strHN = Split(Mid(Result,20),vbCr)(1)	'第2种方法，用Mid函数提取，在用Split提取R1  
	   msgbox Split(Mid(Result,20),vbCr)(2)   '该数组中第一个元素为空
	   msgbox  strHN
	   HN = Mid(StrHN,11)
